...
Thing's like **A-Star** which is an algorithm to find the shortest path between a start and a specified goal. It does this by taking an estimate how long the shortest path. **A-Star** at it's base is a depth first search meaning so what does this actually mean if you think of a graph and pick a point on the graph all of the points directly around that point would be considered a a child of that point. Those children then also have children depth first search means instead of searching all the points directly next to the point it picks one of the children and searches there and it repeats until it gets to the end of the tree or when there are no more possible moves on that branch. **A-Star** Has an extra step it takes the estimated path-distance and compares it to the nodes it then assigns each node a value based on how much it estimates the path to be if it picks that node It then selects the node with the lowest value to search first[@AStarSearch].

### Djikstra Algorithm

should be this and Djikstra and a lot of the more fundamental algorithms can still be used in this situation the states or weight of an action just need to be evaluated differently and the way information is provided is different.
...
There are also API's such as SDL/SDL2/SDl3 and something like Microsoft DirectX which are are libraries built for interacting with a computers hardware to display images, create windows, play sound and track inputs. These are often used to make simple and high-performance games directly while also often being used as an important component for more complex and powerful game engines[@SDLSITE].
...
**Feedback**
No assumptions are addressed. Talk about the assumptions you are making, what does your tool rely on, how could that limit its development or functionality?
**Feedback**
