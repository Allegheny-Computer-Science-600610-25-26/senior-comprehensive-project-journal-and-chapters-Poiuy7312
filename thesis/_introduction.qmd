...
The type of algorithm you want to use and whether you want to use a navigation mesh or a probabilistic map, or simply to a straight algorithm depends on the need and resources available for the project. A nav mesh represents the walkable areas as polygons and connects these areas together using A* to help create paths calculating much smaller paths in subsections of the map instead of trying to calculate the whole map at once. Probabilistic maps are very similar but instead of being represented by polygons it's individual node and each node tries to connect to each other and tries to find paths without colliding with each other.



### Basic 2D Grid

This tool will only be working with a 2D unweighted grid. One because it's the easiest to implement and for the purposes of analyzing a path-finding algorithm. Probabilistic Maps and Nav-meshes are generated from path-finding algorithms and stored for later in a static environment like a video-game map.

## Project Overview

This project presents a tool designed to be easy to use and understand to help in the development and analysis of pathfinding algorithms. The tool allows users to create, edit, generate and save maps. Supporting the customization of test environments for algorithm evaluation. The program analyzes the results through benchmarking by recording metrics such as initial distance from the goal, steps taken to reach the goal, computational steps required for a solution, and memory usage. It then stores this information and displays it for analysis. The hope for this project is that it makes testing and analyzing path-finding algorithms easier and to have a dedicated tool to that purpose instead of using a secondary feature of something like a game engine or having to make your own testing environment.

### Rust

The tool is created using the Rust programming language this is do to Rust's speed, speed memory safety, focus on concurrency and the integrated package manager cargo making it easy to develop and use different libraries or how to there referred to as crates

### SDL2

In order to handle inputs and the display of the UI this tools relies on the rust bindings for SDL2. SDL2 is a multimedia library originally written in C. The reason for using SDL is because of the amount of control it gives to the developer as result of how low level it is. While also being consistently updated and compatible with many different operating systems.
...
Game Engines like Unreal, Godot, Unity and others are often used as testbeds for path-finding algorithms as they can provide bench marking and the ability to create and control their own environments. However, this isn't their main purpose and game engines can be difficult to use, especially something like the unreal developed by epic games. This leads to an issue where a tool isn't powerful enough to be used as a proper way to analyze something or it has a major learning curve on how to create a map and implement code because of it having a broader purpose and not being focused on analyzing path-finding algorithms.
